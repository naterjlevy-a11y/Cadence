...
@main
final class MellotronApp: NSObject, NSApplicationDelegate {
    static func main() {
        let app = NSApplication.shared
        let delegate = MellotronApp()
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        Log.app.info("Mellotron starting up at \(Bundle.main.bundlePath, privacy: .public)")
        ...
        // Warm the Mellotron Cloud session a beat after launch so the first
        // dictation doesn't pay the anonymous-signup round trip inline.
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.2) {
            AuthService.shared.ensureCloudSessionReady { _ in }
        }

        // Catch mellotron://auth/callback redirects from OAuth providers.
        ...
        NotificationCenter.default.addObserver(
            forName: .mellotronOpenPermissionRepair,
            object: nil,
            queue: .main
        ) { [weak self] _ in
        ...
        NotificationCenter.default.addObserver(
            forName: NSNotification.Name("MellotronInputMonitoringGranted"),
            object: nil,
            queue: .main
        ) { [weak self] _ in
            self?.hotkeys.start()
    ...
    private func installMainMenu() {
        ...
        let appMenu = NSMenu()
        appMenu.addItem(NSMenuItem(title: "About Mellotron",
        ...
        appMenu.addItem(.separator())
        appMenu.addItem(NSMenuItem(title: "Hide Mellotron",
                                   action: #selector(NSApplication.hide(_:)),
                                   keyEquivalent: "h"))
        appMenu.addItem(.separator())
        appMenu.addItem(NSMenuItem(title: "Quit Mellotron",
    ...
    @objc private func handleGetURLEvent(_ event: NSAppleEventDescriptor, withReplyEvent reply: NSAppleEventDescriptor) {
        ...
        guard url.scheme?.lowercased() == "mellotron" else { return }
...
extension MellotronApp: HotkeyManagerDelegate {
    func hotkeyDidPress() {
