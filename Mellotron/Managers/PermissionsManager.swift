...
/// Tracks the macOS permissions Mellotron depends on and exposes
...
final class PermissionsManager: ObservableObject {
    ...
    func openAccessibilitySettings() {
        // First request a prompt so macOS adds Mellotron to the list.
    ...
    func requestInputMonitoring() {
        // Triggers the system prompt to add Mellotron to the list.
    ...
    func resetAndReprompt(_ kind: TCCKind) -> Bool {
        let bundleId = Bundle.main.bundleIdentifier ?? "com.mellotron.Mellotron"
        let task = Process()
...
/// The small set of TCC services Mellotron talks to. The raw string is
