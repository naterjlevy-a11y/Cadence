...
extension Notification.Name {
    ...
    static let mellotronUserCancelled = Notification.Name("mellotron.user.cancelled")
    /// Posted when hybrid mode latches into hands-free recording.
    static let mellotronDidLatch = Notification.Name("mellotron.didLatch")
    /// Posted when hybrid mode unlatches.
    static let mellotronDidUnlatch = Notification.Name("mellotron.didUnlatch")
    ...
    static let mellotronShowMenuPopover = Notification.Name("mellotron.showMenuPopover")
    /// Posted to show a transient corner toast. `userInfo["message"]` is the text.
    static let mellotronShowToast = Notification.Name("mellotron.showToast")
}
...
final class DictationCoordinator: ObservableObject {
    ...
    func cancel() {
        ...
        stage = .idle
        lastError = nil
        NotificationCenter.default.post(name: .mellotronUserCancelled, object: nil)
    }
    ...
    private static func isSilentRecording(peak: Float, duration: TimeInterval) -> Bool {
        if peak < -48 { return true }                       // essentially silent
        if duration < 0.7 && peak < -40 { return true }     // quick quiet misfire tap
        return false
    }
