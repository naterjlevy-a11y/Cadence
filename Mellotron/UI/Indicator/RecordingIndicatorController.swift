...
final class RecordingIndicatorController {
    ...
    // Compact Wispr-Flow-style pill, Mellotron-tinted.
    ...
    func install() {
        ...
        NotificationCenter.default.publisher(for: .mellotronShowToast)
            .receive(on: DispatchQueue.main)
        ...
        NotificationCenter.default.publisher(for: .mellotronUserCancelled)
            .receive(on: DispatchQueue.main)
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            rightBadge
                ...
                .contentShape(Circle())
                .onTapGesture {
                    NotificationCenter.default.post(name: .mellotronShowMenuPopover, object: nil)
                }
                .help("Open Mellotron menu")
        }
        ...
        .onReceive(NotificationCenter.default.publisher(for: .mellotronDidLatch)) { _ in
            isLatched = true
        }
        .onReceive(NotificationCenter.default.publisher(for: .mellotronDidUnlatch)) { _ in
            isLatched = false
        }
    }
