...
private struct PopupSettingsSection: View {
    ...
    var body: some View {
        SettingsCard(
            "Popup window",
            subtitle: "Small, top-right, always on top. When Chrome session sync is on, Mellotron copies your sign-in cookies from Chrome before each popup loads."
        ) {
            SettingsRow(
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 22) {
            ...
            SettingsCard("Activation mode", subtitle: prefs.activationMode.description) {
                SettingsRow("Pause voice routing", detail: prefs.paused ? "Mellotron is paused — push-to-talk is ignored." : "Push-to-talk is active.") {
            ...
            SettingsCard("Developer") {
                ...
                SettingsRow("Sentry test event", detail: "Sends a harmless message to your Sentry project.") {
                    Button("Send test") {
                        CrashReporter.captureMessage("Mellotron manual test event from Settings")
                    }
            ...
            .confirmationDialog(
                ...
                Text("Restores Mellotron to factory defaults. Your Groq key in Keychain or ~/.config/mellotron/.env is not removed.")
            }
        }
...
private struct DictationSettingsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 22) {
            ...
            SettingsCard(
                "Personal dictionary",
                subtitle: "Add names, brands, or jargon that get misheard. Mellotron keeps these spelled exactly the way you type them."
            ) {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if prefs.personalDictionary.isEmpty {
                Text("No custom words yet. Try adding your name, a company, or a product Mellotron keeps mishearing.")
                    .font(.system(size: 12))
                    .foregroundStyle(.secondary)
            } else {
...
private struct RoutingExplainerCard: View {
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("Start talking with \u{201C}Hey\u{201D} and a name to send your words somewhere specific. Without a greeting, Mellotron just types into the app you're already in.")
                .font(.melloBody(12.5))
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
        ...
        VStack(alignment: .leading, spacing: 18) {
            SettingsCard(destination.displayName) {
                ...
                if supportsPopup {
                    SettingsRow(
                        "Open in popup",
                        detail: "Show this destination in Mellotron's floating mini-browser instead of switching apps."
                    ) {
            ...
            SettingsCard(
                "Document shortcuts",
                subtitle: "Add specific URLs you can target by name. Say \"Hey \(destination.displayName), [shortcut name], …\" and Mellotron opens that URL instead."
            ) {
...
private struct GeneralPrivacySection: View {
    ...
    var body: some View {
        Group {
            ...
            SettingsCard("About") {
                Text("Mellotron transcribes speech using Groq Whisper (cloud) or Apple Speech (on-device). Audio is only sent while the activation key is engaged.")
                    .font(.system(size: 12))
                    .foregroundStyle(.secondary)
            }
            SettingsCard(
                "macOS permissions",
                subtitle: "If a toggle looks ON in System Settings but Mellotron still says denied, use Repair below — it clears the stale entry so the next prompt registers this build."
            ) {
                PermissionRowsView()
            ...
            if !duplicates.isEmpty {
                SettingsCard(
                    ...
                    subtitle: "Extra copies of Mellotron confuse macOS permissions. Move them to the Trash."
                ) {
...
private struct HelpFeedbackSection: View {
    ...
    private func sendFeedback() {
        ...
        let subject = "Mellotron \(version) (\(build)) feedback"
        let body = "\n\n—\nApp: Mellotron \(version) (\(build))\nmacOS: \(os)\n"
        let encodedSubject = subject.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? subject
        let encodedBody = body.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? body
        if let url = URL(string: "mailto:support@mellotron.me?subject=\(encodedSubject)&body=\(encodedBody)") {
...
// MARK: - Subscription (Mellotron Pro)
...
private struct TranscriptionSettingsSection: View {
    ...
    var body: some View {
        SettingsCard(
            "Transcription",
            subtitle: "Speech-to-text is powered by Mellotron Cloud (Groq Whisper) — no setup or API key needed."
        ) {
            SettingsRow(
    ...
    private func friendlyProvider(_ raw: String) -> String {
        switch raw {
        case "MellotronCloud": return "Mellotron Cloud"
