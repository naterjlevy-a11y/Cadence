...
final class MenuBarController: NSObject {
    ...
    func install() {
        ...
        NotificationCenter.default.publisher(for: .mellotronShowMenuPopover)
            .receive(on: DispatchQueue.main)
    ...
    private func renderIcon(for stage: DictationStage) -> NSImage {
        ...
        let baseY = rect.minY + 1
        let topY = rect.maxY - 1
        let midY = rect.midY
        let leftX = rect.minX
        let midX = rect.midX
        let rightX = rect.maxX
        path.move(to: NSPoint(x: leftX, y: baseY))
        path.line(to: NSPoint(x: leftX, y: topY))
        path.line(to: NSPoint(x: midX, y: midY))
        path.line(to: NSPoint(x: rightX, y: topY))
        path.line(to: NSPoint(x: rightX, y: baseY))
        path.lineWidth = 1.6
        ...
        default: stroke = NSColor(red: 0.72, green: 0.56, blue: 1.0, alpha: 1.0)
        }
        stroke.setStroke()
        path.stroke()
        ...
        if stage == .recording {
            ...
            let dot = NSBezierPath(ovalIn: NSRect(x: rightX - 3, y: topY - 3, width: 4, height: 4))
            dot.fill()
        }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(spacing: 1) {
                ...
                rowButton(title: "Quit Mellotron", glyph: "power", action: onQuit)
            }
