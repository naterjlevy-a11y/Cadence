...
final class AIPolishProvider {
    ...
    /// True when polish can run: either a personal key is set, or Mellotron
    ...
    /// Polish via Mellotron Cloud — the Worker holds the LLM key and returns
    ...
    func polish(
        ...
        // No personal key → use Mellotron Cloud (server holds the LLM key).
        ...
        2. Fix obvious transcription errors and brand-name mistranscriptions:
           "Claudia"/"Claudio" → Claude · "Jiminy"/"Germany"/"Gemany" → Gemini · "Chad GPT" → ChatGPT · "Courser"/"Kurser" → Cursor · "wisper flow"/"whisper flow" → Wispr Flow · "mellow tron" → Mellotron · "mick gill" → McGill · "MIT" stays MIT · "youtube" → YouTube.
    ...
    private func sendPolishRequest(
        ...
        req.setValue("https://mellotron.app", forHTTPHeaderField: "HTTP-Referer")
        req.setValue("Mellotron", forHTTPHeaderField: "X-Title")
        req.httpBody = body
