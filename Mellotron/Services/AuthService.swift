...
///   5. `handleCallbackURL`          — receives `mellotron://auth/...` from OAuth
final class AuthService: NSObject, ObservableObject {
    ...
    /// Bearer token for Mellotron Cloud API calls.
    ...
    /// Ensures a Mellotron Cloud bearer token exists before Groq-via-proxy calls.
    ...
    func setPassword(_ password: String, completion: @escaping (Result<Void, Error>) -> Void) {
        ...
        URLSession.shared.dataTask(with: req) { [weak self] data, response, _ in
            DispatchQueue.main.async {
                ...
                NotificationCenter.default.post(name: .mellotronAuthCompleted, object: nil)
                completion(.success(()))
            }
        }.resume()
    }
    ...
    /// hand off to Google/GitHub/etc., then redirect to `mellotron://auth/callback`.
    func signInWithProvider(_ provider: OAuthProvider) {
        guard let base = CloudConfig.shared.supabaseURL else {
            lastError = "Mellotron Cloud is not configured."; return
        }
        let redirect = CloudConfig.shared.authRedirectURL
    ...
    /// Called when macOS opens `mellotron://auth/callback…` (from the web bridge
    ...
    private func applySessionTokens(access: String, refresh: String) {
        ...
        fetchQuota()
        NSApp.activate(ignoringOtherApps: true)
        Log.app.info("Mellotron Cloud session active (callback)")
        NotificationCenter.default.post(name: .mellotronAuthCompleted, object: nil)
    }
    ...
    func signOut() {
        ...
        refreshTimer = nil
        Log.app.info("Signed out of Mellotron Cloud")
    }
    ...
    func fetchQuota() {
        ...
        URLSession.shared.dataTask(with: req) { [weak self] data, _, _ in
            ...
            DispatchQueue.main.async { self?.plan = plan }
        }.resume()
    }
    ...
    private func applyAuthResponse(data: Data?, response: URLResponse?, source: AuthSource, completion: @escaping (Bool) -> Void) {
        ...
        scheduleRefresh()
        fetchQuota()
        Log.app.info("Mellotron Cloud session active (source=\(String(describing: source), privacy: .public))")
        completion(true)
    }
...
enum AuthError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notConfigured: return "Mellotron Cloud isn’t configured."
...
extension Notification.Name {
    static let mellotronAuthCompleted = Notification.Name("mellotronAuthCompleted")
}
...
extension AuthService: ASAuthorizationControllerDelegate {
    func authorizationController(controller: ASAuthorizationController, didCompleteWithAuthorization authorization: ASAuthorization) {
        ...
        exchangeAppleToken(idToken: idToken) { [weak self] ok in
            ...
            if ok { NotificationCenter.default.post(name: .mellotronAuthCompleted, object: nil) }
        }
    }
