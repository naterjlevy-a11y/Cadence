...
enum TranscriptionVocabulary {
    ...
    static func whisperPrompt() -> String {
        let priority: [String] = [
            "Mellotron, Wispr Flow, McGill, Claude, Anthropic, ChatGPT, OpenAI, Cursor, Gemini, GitHub, TypeScript, Node.js, Next.js, VS Code, Xcode, macOS, iOS, Formula SAE, Formula Electric",
        ]
...
private enum WhisperMultipart {
    static func buildBody(fileURL: URL, model: String, prompt: String) throws -> (Data, String) {
        ...
        let boundary = "----MellotronBoundary\(UUID().uuidString)"
        var body = Data()
...
// MARK: - Mellotron Cloud (Groq via your proxy)

/// Sends audio to your Cloudflare Worker; the server holds the Groq key.
final class MellotronCloudTranscriptionProvider: TranscriptionProvider {
    let displayName = "Mellotron Cloud"
    ...
    func transcribe(fileURL: URL, completion: @escaping (Result<TranscriptionResult, Error>) -> Void) {
        ...
        guard let endpoint = CloudConfig.shared.transcribeURL else {
            completion(.failure(DictationError.transcriptionFailed("Mellotron Cloud is not configured.")))
            return
        }
        let token = AuthService.shared.accessToken ?? ""
        guard !token.isEmpty else {
            completion(.failure(DictationError.transcriptionFailed("Sign in to Mellotron Cloud in Settings → Account, or use your own Groq key.")))
            return
        }
        ...
        let task = URLSession.shared.dataTask(with: req) { [weak self] data, response, error in
            ...
            if status == 429 {
                completion(.failure(DictationError.transcriptionFailed("Monthly Mellotron Cloud limit reached. Upgrade or add your own Groq key in Settings.")))
                return
            }
            if status >= 400 {
            ...
            Log.transcription.info("Mellotron Cloud ok in \(elapsedMs, privacy: .public)ms")
            completion(.success(TranscriptionResult(text: cleaned, confidence: 1.0, provider: "MellotronCloud")))
        }
        lock.lock()
        currentTask = task
        lock.unlock()
...
final class TranscriptionService {
    ...
    private let cloudProvider = MellotronCloudTranscriptionProvider()
