...
/// injects them into Mellotron's WKWebView popup so sites open already signed in.
final class ChromeCookieImporter {
    ...
    private let queue = DispatchQueue(label: "com.mellotron.chrome-cookies", qos: .userInitiated)
    ...
    private func readCookieRows(from dbURL: URL, host: String) -> [CookieRow]? {
        ...
        let tempURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("mellotron-chrome-cookies-\(UUID().uuidString).db")
