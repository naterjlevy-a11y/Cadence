...
struct CloudConfig {
    ...
    /// True when Supabase + API base are configured (enables Mellotron Cloud).
    ...
    private init() {
        ...
        var api: URL?
        var dsn = ""
        var authRedirect = "mellotron://auth/callback"
        var stripePK = ""
        ...
        if let envURL = ProcessInfo.processInfo.environment["MELLOTRON_SUPABASE_URL"],
           let parsed = URL(string: envURL) {
            url = parsed
        }
        if let key = ProcessInfo.processInfo.environment["MELLOTRON_SUPABASE_ANON_KEY"], !key.isEmpty {
            anonKey = key
        }
        if let envAPI = ProcessInfo.processInfo.environment["MELLOTRON_API_BASE_URL"],
           let parsed = URL(string: envAPI) {
            api = parsed
        }
        if let envDSN = ProcessInfo.processInfo.environment["MELLOTRON_SENTRY_DSN"], !envDSN.isEmpty {
            dsn = envDSN
        }
        if let envRedirect = ProcessInfo.processInfo.environment["MELLOTRON_AUTH_REDIRECT_URL"], !envRedirect.isEmpty {
            authRedirect = envRedirect
        }
        if let envPK = ProcessInfo.processInfo.environment["MELLOTRON_STRIPE_PUBLISHABLE_KEY"], !envPK.isEmpty {
            stripePK = envPK
        }
